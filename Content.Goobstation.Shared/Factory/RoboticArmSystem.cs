...
public sealed partial class RoboticArmSystem : EntitySystem
{
    ...
    public bool TryPickupAny(Entity<RoboticArmComponent> ent)
    {
        ...
        if (count > 0)
        {
            ...
            for (var i = count - 1; i >= 0; i--)
            {
                ...
                if (output != null && !output.CanInsert(item.Value))
                    continue;

                ent.Comp.InputItems.RemoveAt(i);
                DirtyField(ent, ent.Comp, nameof(RoboticArmComponent.InputItems));

                _wake.SetEnabled(item.Value, false);

                if (!_slots.TryInsert(ent, ent.Comp.ItemSlot, item.Value, user: null))
                    return false;
    ...
    private void UpdateItemSlots(Entity<RoboticArmComponent> ent)
    {
        ...
        if (!_slots.TryGetSlot((ent.Owner, slots), ent.Comp.ItemSlotId, out var slot))
        {
    ...
    private void SetPowerDraw(EntityUid uid, float draw)
    {
        ...
        {
            Entity<SharedApcPowerReceiverComponent?> entity = new(uid, receiver);
            _power.SetLoad(entity, draw);
        }
    }
